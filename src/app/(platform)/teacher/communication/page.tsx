...
export default function TeacherCommunicationPage() {
  ...
  const fetchAnnouncements = useCallback(async () => {
    try {
      const res = await fetch("/api/announcements?published=true");
      if (res.ok) {
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    fetchStaff();
    fetchCurrentUser();
  }, [fetchMessages, fetchAnnouncements, fetchStaff, fetchCurrentUser]);
  ...
  const inboxMessages = filtered.filter((m) => m.recipientId === currentUserId);
  const unreadCount = messages.filter(
    (m) => !m.readAt && m.recipientId === currentUserId
  ).length;

  const handleReply = (msg: Message) => {
    const senderStaff = staff.find((s) => s.userId === msg.senderId);
    setReplyRecipientId(msg.senderId ?? "");
    setReplySubject(msg.subject?.startsWith("Re: ") ? msg.subject : `Re: ${msg.subject || "No subject"}`);
    setReplyContent("");
